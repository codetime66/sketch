...
// START getFollowRedirects
    public Object getContent() throws IOException {
	connect();  // make sure we're connected

	int code = getResponseCode(); // get response code
	if (getFollowRedirects() && (code/100) == 3) {
	    String newLocation = getHeaderField("location");
	    if (newLocation != null) {
		// Very simplistic assumption: use HTTP again
		HttpImpl n = new HttpImpl(new URL(newLocation));
		return n.getContent();
	    }
	}
	// else just continue as before
...
// END
	...
	if (!connected)
	    throw new IOException("Connection not established yet");
